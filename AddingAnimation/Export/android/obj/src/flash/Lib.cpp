			...
			HX_STACK_ARG(frameHandle,"frameHandle");
			{
				...
				try{
				...
				catch(Dynamic __e){
					{
						...
						Dynamic e = __e;{
							HX_STACK_LINE(107)
							::haxe::Log_obj::trace((HX_CSTRING("ERROR: ") + ::Std_obj::string(e)),hx::SourceInfo(HX_CSTRING("Lib.hx"),107,HX_CSTRING("flash.Lib"),HX_CSTRING("create")));
						}
					}
				}
			}
...
Void Lib_obj::postUICallback( Dynamic inCallback){
{
		...
		HX_STACK_LINE(376)
		::flash::Lib_obj::nme_post_ui_callback(inCallback);
	}
return null();
}
...
void Lib_obj::__boot()
{
	...
	__moduleNames= null();
	__stage= null();
	nme_post_ui_callback= ::flash::Lib_obj::load(HX_CSTRING("nme"),HX_CSTRING("nme_post_ui_callback"),(int)1);
