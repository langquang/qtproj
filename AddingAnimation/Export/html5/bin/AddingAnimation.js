...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("assets/B11S2Building.png",loader);
	ApplicationMain.total++;
	var loader1 = new flash.display.Loader();
	ApplicationMain.loaders.set("assets/openfl.png",loader1);
	ApplicationMain.total++;
...
var Main = function() {
	...
	var bmData = openfl.Assets.getBitmapData("assets/B11S2Building.png");
	...
	container.set_x(this.get_stage().get_stageWidth() / 2);
	container.set_y(this.get_stage().get_stageHeight() / 2);
	this.addChild(container);
	this.tf = new flash.text.TextField();
	this.tf.set_text("Butin");
	this.addChild(this.tf);
	var filter = new flash.filters.GlowFilter(16711680,1,10,10,5);
	var bmFilter = new flash.display.BitmapData(bmData._nmeTextureBuffer != null?bmData._nmeTextureBuffer.width:0,bmData._nmeTextureBuffer != null?bmData._nmeTextureBuffer.height:0);
	bmFilter.applyFilter(bmData,bmData.rect,new flash.geom.Point(0,0),filter);
	bitmap.set_bitmapData(bmFilter);
	var mStarling = new qtframework.qtcore.Starling(this.get_stage());
	mStarling.start();
	var delay = mStarling.get_juggler().delayCall($bind(this,this.delay3s),1,null);
	delay.set_repeatCount(2);
	delay.addEventListener("removeFromJuggler",$bind(this,this.onRemove));
};
$hxClasses["Main"] = Main;
Main.__name__ = ["Main"];
...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	onRemove: function(event) {
		this.tf.set_text("Remove Delay");
	}
	,delay3s: function(args) {
		this.tf.set_text(Std.string(this.count));
		this.count++;
	}
	,onEnterFrame: function(event) {
		var t1 = flash.Lib.getTimer();
		this.tf.set_text(Std.string(t1));
	}
	,__class__: Main
});
...
nme.AssetData.initialize = function() {
	if(!nme.AssetData.initialized) {
		nme.AssetData.path.set("assets/B11S2Building.png","assets/B11S2Building.png");
		var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
		nme.AssetData.type.set("assets/B11S2Building.png",value);
		nme.AssetData.path.set("assets/openfl.png","assets/openfl.png");
		var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
		nme.AssetData.type.set("assets/openfl.png",value);
...
var qtframework = {}
qtframework.events = {}
qtframework.events.QTEvent = function(type,data,bubbles) {
	if(bubbles == null) bubbles = false;
	flash.events.Event.call(this,type,bubbles);
	this.mData = data;
};
$hxClasses["qtframework.events.QTEvent"] = qtframework.events.QTEvent;
qtframework.events.QTEvent.__name__ = ["qtframework","events","QTEvent"];
qtframework.events.QTEvent.__super__ = flash.events.Event;
qtframework.events.QTEvent.prototype = $extend(flash.events.Event.prototype,{
	__class__: qtframework.events.QTEvent
});
qtframework.qtanimation = {}
qtframework.qtanimation.IAnimatable = function() { }
$hxClasses["qtframework.qtanimation.IAnimatable"] = qtframework.qtanimation.IAnimatable;
qtframework.qtanimation.IAnimatable.__name__ = ["qtframework","qtanimation","IAnimatable"];
qtframework.qtanimation.IAnimatable.prototype = {
	__class__: qtframework.qtanimation.IAnimatable
}
qtframework.qtanimation.DelayedCall = function(func,delay,args) {
	this.mRepeatCount = 1;
	this.mCurrentTime = 0;
	flash.events.EventDispatcher.call(this);
	this.mCall = func;
...
$hxClasses["qtframework.qtanimation.DelayedCall"] = qtframework.qtanimation.DelayedCall;
qtframework.qtanimation.DelayedCall.__name__ = ["qtframework","qtanimation","DelayedCall"];
qtframework.qtanimation.DelayedCall.__interfaces__ = [qtframework.qtanimation.IAnimatable];
qtframework.qtanimation.DelayedCall.__super__ = flash.events.EventDispatcher;
qtframework.qtanimation.DelayedCall.prototype = $extend(flash.events.EventDispatcher.prototype,{
	...
	,advanceTime: function(time) {
		...
		if(previousTime < this.mTotalTime && this.mCurrentTime >= this.mTotalTime) {
			...
			} else this.dispatchEvent(new qtframework.events.QTEvent("removeFromJuggler"));
		}
	}
	,__class__: qtframework.qtanimation.DelayedCall
});
qtframework.qtanimation.Juggler = function() {
	this.mElapsedTime = 0;
	this.mObjects = new Array();
};
$hxClasses["qtframework.qtanimation.Juggler"] = qtframework.qtanimation.Juggler;
qtframework.qtanimation.Juggler.__name__ = ["qtframework","qtanimation","Juggler"];
qtframework.qtanimation.Juggler.__interfaces__ = [qtframework.qtanimation.IAnimatable];
qtframework.qtanimation.Juggler.prototype = {
	onRemove: function(event) {
	...
	,advanceTime: function(time) {
		...
		var _g1 = 0, _g = this.mObjects.length;
		while(_g1 < _g) {
			var i1 = _g1++;
			var object = this.mObjects[i1];
			if(object == null) this.mObjects.splice(i1,1); else object.advanceTime(time);
		}
	}
	,delayCall: function(func,delay,args) {
		if(func == null) return null;
		var delayedCall = new qtframework.qtanimation.DelayedCall(func,delay,args);
	...
	,remove: function(object) {
		...
		HxOverrides.remove(this.mObjects,object);
	}
	,add: function(object) {
		if(object != null && Lambda.indexOf(this.mObjects,object) == -1) {
	...
	,__class__: qtframework.qtanimation.Juggler
}
qtframework.qtcore = {}
qtframework.qtcore.Starling = function(stage) {
	flash.events.EventDispatcher.call(this);
	if(stage == null) throw "Stage must not be null";
	this.mNativeOverlay = new flash.display.Sprite();
	this.mNativeStage = stage;
	this.mNativeStage.addChild(this.mNativeOverlay);
	this.mViewPort = new flash.geom.Rectangle(0,0,stage.get_stageWidth(),stage.get_stageHeight());
	this.mJuggler = new qtframework.qtanimation.Juggler();
	this.mLastFrameTimestamp = this.getSecond();
	stage.addEventListener(flash.events.Event.ENTER_FRAME,$bind(this,this.onEnterFrame));
	stage.addEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(this,this.onKey));
	stage.addEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(this,this.onKey));
	stage.addEventListener(flash.events.Event.RESIZE,$bind(this,this.onResize));
};
$hxClasses["qtframework.qtcore.Starling"] = qtframework.qtcore.Starling;
qtframework.qtcore.Starling.__name__ = ["qtframework","qtcore","Starling"];
qtframework.qtcore.Starling.__super__ = flash.events.EventDispatcher;
qtframework.qtcore.Starling.prototype = $extend(flash.events.EventDispatcher.prototype,{
	getSecond: function() {
	...
	,dispose: function() {
		this.stop();
		this.mNativeStage.removeEventListener(flash.events.Event.ENTER_FRAME,$bind(this,this.onEnterFrame),false);
		this.mNativeStage.removeEventListener(flash.events.KeyboardEvent.KEY_DOWN,$bind(this,this.onKey),false);
		this.mNativeStage.removeEventListener(flash.events.KeyboardEvent.KEY_UP,$bind(this,this.onKey),false);
		this.mNativeStage.removeEventListener(flash.events.Event.RESIZE,$bind(this,this.onResize),false);
		if(qtframework.qtcore.Starling.sCurrent == this) qtframework.qtcore.Starling.sCurrent = null;
	}
	,__class__: qtframework.qtcore.Starling
});
...
qtframework.events.QTEvent.ADDED = "added";
qtframework.events.QTEvent.ADDED_TO_STAGE = "addedToStage";
qtframework.events.QTEvent.ENTER_FRAME = "enterFrame";
qtframework.events.QTEvent.REMOVED = "removed";
qtframework.events.QTEvent.REMOVED_FROM_STAGE = "removedFromStage";
qtframework.events.QTEvent.TRIGGERED = "triggered";
qtframework.events.QTEvent.FLATTEN = "flatten";
qtframework.events.QTEvent.RESIZE = "resize";
qtframework.events.QTEvent.COMPLETE = "complete";
qtframework.events.QTEvent.CONTEXT3D_CREATE = "context3DCreate";
qtframework.events.QTEvent.ROOT_CREATED = "rootCreated";
qtframework.events.QTEvent.REMOVE_FROM_JUGGLER = "removeFromJuggler";
qtframework.qtcore.Starling.VERSION = "1.2";
